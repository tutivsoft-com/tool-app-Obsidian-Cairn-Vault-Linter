...
var CairnVaultLinterPlugin = class extends import_obsidian.Plugin {
  ...
  async onload() {
    ...
    this.addCommand({ id: "scan-changed-notes", name: "Cairn: Scan changed notes (incremental)", callback: () => void this.runScan(void 0, true) });
  ...
  async applyRepairPlans(plans) {
    const journal = { batchId: `cairn-${Date.now()}`, createdAt: (/* @__PURE__ */ new Date()).toISOString(), entries: plans.map((plan) => ({ path: plan.path, before: plan.before, after: plan.after })) };
...
var CairnSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("Reset Cairn settings").setDesc("Restore default checks and folders; ignored findings are also cleared.").addButton((button) => button.setButtonText("Reset").onClick(async () => {
      this.plugin.settings = mergeSettings(null);
      await this.plugin.saveData(this.plugin.settings);
      this.display();
